#!/usr/bin/env python3
"""Unified ANSI EXPRESSION_OFFLOAD log analyzer.
...
Usage:
    python3 dev/analyze-expression_offload.py /tmp/ansi-matrix/latest/
    python3 dev/analyze-expression_offload.py /tmp/ansi-matrix/latest/all-ut-spark41.log
    python3 dev/analyze-expression_offload.py /tmp/ansi-matrix/latest/cast-ut-spark41.log
"""
import argparse
import glob
import os
...
# EXPRESSION_OFFLOAD summary patterns (pipe-separated, indented)
EXPRESSION_OFFLOAD_SUMMARY_BEGIN = 'EXPRESSION_OFFLOAD_SUMMARY_BEGIN'
EXPRESSION_OFFLOAD_SUMMARY_END = 'EXPRESSION_OFFLOAD_SUMMARY_END'
EXPRESSION_OFFLOAD_RECORD_RE = re.compile(r'^\s+([NE])\|(.+)\|(.+)\|(\w+)\|(\w+)$')
EXPRESSION_OFFLOAD_SUITE_RE = re.compile(r'^(\w+(?:Suite|Test)):$')
EXPRESSION_OFFLOAD_TEST_RE = re.compile(r'^\s{2}(.+):$')
...
SUITE_CATEGORY = {
    ...
    # No EXPRESSION_OFFLOAD, test results only
...
def parse_log(path):
    """Parse a single log file → (test_results, expression_offload_entries, lines)."""
    ...
    # Phase 2: EXPRESSION_OFFLOAD summary
    expression_offload_entries = []
    in_summary = False
    cur_suite = None
    cur_test = None
    for i, line in enumerate(lines):
        ...
        if EXPRESSION_OFFLOAD_SUMMARY_BEGIN in line:
            in_summary = True
            continue
        if EXPRESSION_OFFLOAD_SUMMARY_END in line:
            in_summary = False
        ...
        m = EXPRESSION_OFFLOAD_SUITE_RE.match(content)
        if m:
            cur_suite = m.group(1)
            continue
        m = EXPRESSION_OFFLOAD_TEST_RE.match(content)
        if m:
            cur_test = m.group(1)
            continue
        m = EXPRESSION_OFFLOAD_RECORD_RE.match(content)
        if m:
            method_tag, expression, meta, offload, status = m.groups()
            expression_offload_entries.append({
    ...
    return test_results, expression_offload_entries


def load_logs(path):
    """Load from file or directory → merged (test_results, expression_offload_entries)."""
    if os.path.isfile(path):
        return parse_log(path)
    elif os.path.isdir(path):
        all_tests = []
        all_expression_offload = []
        for f in sorted(glob.glob(os.path.join(path, '*-ut-*.log'))):
            t, p = parse_log(f)
            all_tests.extend(t)
            all_expression_offload.extend(p)
        return all_tests, all_expression_offload
    else:
...
def group_by_category(test_results, expression_offload_entries):
    """Split data by category using SUITE_CATEGORY mapping."""
    cat_tests = defaultdict(list)
    cat_expression_offload = defaultdict(list)
    for t in test_results:
        cat = SUITE_CATEGORY.get(t['suite'])
        if cat:
            cat_tests[cat].append(t)
    for p in expression_offload_entries:
        cat = SUITE_CATEGORY.get(p['suite'])
        if cat:
            cat_expression_offload[cat].append(p)
    return cat_tests, cat_expression_offload
...
def analyze_cast(test_results, expression_offload_entries):
    ...
    for e in expression_offload_entries:
...
def print_cast(test_results, expression_offload_entries):
    ...
    success_rows, fallback_rows, failed_rows = analyze_cast(
        test_results, expression_offload_entries)
...
def print_arithmetic(test_results, expression_offload_entries):
...
def print_mechanism_b(category, test_results, expression_offload_entries):
...
def main():
    parser = argparse.ArgumentParser(
        description='Unified ANSI EXPRESSION_OFFLOAD log analyzer')
    parser.add_argument('path', help='Log file or directory containing *-ut-*.log files')
    args = parser.parse_args()

    test_results, expression_offload_entries = load_logs(args.path)
    if not test_results:
        print(f"No tests found in {args.path}", file=sys.stderr)
        sys.exit(1)

    cat_tests, cat_expression_offload = group_by_category(test_results, expression_offload_entries)
    ...
    for cat in CATEGORY_ORDER:
        ...
        tr = cat_tests[cat]
        pe = cat_expression_offload.get(cat, [])
        ...
        if cat == 'cast':
            ...
            # aggregate, errors — no EXPRESSION_OFFLOAD, just test summary
