...
public:
    /*explicit VirtualColumnRowIndexReader(RowGroupInformation& row_group_information, const DB::DataTypePtr & column_type)
    : column_type_(column_type)
    {
        //TODO: row_ranges_provider_;
        row_ranges_provider_ = nullptr;
        for (const auto & row_group : row_group_information)
        {
            row_groups_.push_back(row_group.index);
            row_group_ordinal_to_rowIndex_[row_group.index] = row_group.startRowIdx;
        }
